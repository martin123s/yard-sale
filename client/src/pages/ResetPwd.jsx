...
const ResetPwd = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const user = await resetPassword(verifyCode, password, token)
      setUserMsg(user)
    } catch (err) {
    ...
    <>
      <div className="w-full min-h-screen px-[5%]">
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          <h1 className="text-5xl text-center mb-6 text-teal-700">Reset Password</h1>
          <p className="text-xl font-light mb-8 text-wrap">
            Welcome ! <br/><span className="font-semibold text-red-600">A verification code has been sent to your email for the security reason.</span><br/> Please check your inbox and enter the code below to reset your new password.
          </p>
          ...
          <div className="flex item-center my-10">
            <div className="h-14 w-[65%] border-b-2 border-gray-500 flex justify-between">
              <div className="flex items-center font-semibold text-xl">New Password: </div>
              <div className="h-full flex relative">
                <input
                  value={pwd}
                  onChange={(e) => setPwd(e.target.value)}
                  type={showPwd ? "text" : "password"}
                  className="w-96 text-2xl text-center text-teal-700 font-bold border border-gray-400 rounded focus:outline-none focus:border-gray-400"
                />
